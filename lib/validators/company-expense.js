...
export function validatePatchInvoiceBody(body) {
  if ('invoiceDate' in body && (!body.invoiceDate || !DATE_RE.test(body.invoiceDate))) {
    return '發票日期格式錯誤（應為 YYYY-MM-DD）';
  }
  if ('period' in body && body.period && !PERIOD_RE.test(body.period)) {
    return 'period 格式錯誤，應為民國年.期別，例：113.3-4';
  }
  const hasAnyAmount = 'amount' in body || 'taxAmount' in body || 'totalAmount' in body;
  if (hasAnyAmount) {
    const amt   = Number(body.amount    ?? 0);
    const tax   = Number(body.taxAmount ?? 0);
    const total = Number(body.totalAmount ?? 0);
    if (amt < 0 || tax < 0 || total < 0) return '金額不可為負數';
    if ('amount' in body && 'taxAmount' in body && 'totalAmount' in body) {
      if (Math.abs(amt + tax - total) > 0.5) {
        return `totalAmount(${total}) 應等於 amount(${amt}) + taxAmount(${tax})`;
      }
    }
