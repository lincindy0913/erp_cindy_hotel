...
/**
 * Expand a warehouse name to include itself AND all child storage locations.
 * If the given name is a building (type='building'), returns [buildingName, ...childNames].
 * If it's a storage location or not found, returns [warehouseName].
 * Returns null if warehouseName is falsy (= no filter).
 *
 * @param {object} prisma - Prisma client
 * @param {string|null} warehouseName
 * @returns {Promise<string[]|null>}
 */
export async function expandWarehouseNames(prisma, warehouseName) {
  ...
  try {
    ...
    if (!wh) return [warehouseName];
    if (wh.type === 'building' && wh.children.length > 0) {
      return [warehouseName, ...wh.children.map(c => c.name)];
    }
...
/**
 * Build a Prisma `{ in: [...] }` or exact-match condition from expanded warehouse names.
 * @param {string[]|null} names - result of expandWarehouseNames
 * @returns {string|{in:string[]}|undefined}
 */
export function warehouseWhereValue(names) {
  if (!names) return undefined;
  return names.length === 1 ? names[0] : { in: names };
}
