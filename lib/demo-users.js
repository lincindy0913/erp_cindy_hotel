...
// Default admin user — password is bcrypt-hashed
const DEFAULT_ADMIN_PASSWORD_HASH = bcrypt.hashSync('admin123', 12);
const DEFAULT_ADMIN = {
  id: 1,
...
function verifyPassword(user, password) {
  ...
  // Also handle legacy plain-text passwords by auto-upgrading them
  try {
    if (user.password && user.password.startsWith('$2')) {
      return bcrypt.compareSync(password, user.password);
    }
    // Legacy plain-text fallback: compare then auto-upgrade to bcrypt
    if (user.password === password) {
      try {
        const users = loadUsers();
        const idx = users.findIndex(u => u.id === user.id);
        if (idx !== -1) {
          users[idx].password = bcrypt.hashSync(password, 12);
          saveUsers(users);
        }
