...
export function renderVoucherTablePage(doc, opts) {
  const {
    ...
    // Rich expense items: 費用名稱, 廠商, 館別, 付款方式, 存簿/代墊員工, 摘要, 金額
    expenseItems = [],     // [{ expenseName, supplierName, warehouse, paymentMethod, accountName, summary, amount }]
    cjkFont = undefined,
  } = opts;
  ...
  // ===== Title =====
  const loanSourceTypes = ['loan_predeposit', 'loan_payment'];
  const isLoan = loanSourceTypes.includes(order.sourceType);
  const isExpense = expenseItems.length > 0;
  const typeLabel = isLoan ? '貸款' : isExpense ? '固定費用' : '進貨';
  const titleText = `${warehouseDisplay}  ${typeLabel}  傳 票`;
  doc.setFontSize(18);
  ...
  // ===== SECTION 2: Expense detail (費用明細) =====
  const hasExpenseItems = expenseItems && expenseItems.length > 0;
  const hasExpenseLines = expenseLines && expenseLines.length > 0;

  if (hasExpenseItems || hasExpenseLines) {
    ...
    // ── Rich expense items (費用名稱, 廠商, 館別, 付款方式, 存簿/代墊員工, 摘要, 金額) ──
    if (hasExpenseItems) {
      const head = [['費用名稱', '廠商', '館別', '付款方式', '存簿/代墊員工', '摘要', '金額']];
      const body = expenseItems.map(item => [
        item.expenseName || '',
        item.supplierName || '',
        item.warehouse || '',
        item.paymentMethod || '',
        item.accountName || '',
        item.summary || '',
        Number(item.amount) ? Number(item.amount).toLocaleString() : '',
      ]);
      const itemTotal = expenseItems.reduce((s, i) => s + Number(i.amount || 0), 0);
      body.push(['', '', '', '', '', '合計', itemTotal.toLocaleString()]);
      ...
      doc.autoTable({
        ...
        headStyles: { fillColor: [210, 225, 245], textColor: [30, 30, 30], fontStyle: 'bold', halign: 'center', fontSize: 8, ...(cjkFont && { font: cjkFont }) },
    ...
    // ── Credit lines (貸方明細) — always from expenseLines ──
    if (hasExpenseLines) {
      const creditLines = expenseLines.filter(l => l.entryType === 'credit');
      if (creditLines.length > 0) {
        const head = [['貸方明細', '科目代號', '科目名稱', '摘要', '金額']];
        const body = creditLines.map((l, idx) => [
          String(idx + 1),
          l.accountingCode || '',
          l.accountingName || '',
          l.summary || '',
        ...
        body.push(['', '', '', '小計', creditTotal.toLocaleString()]);
        ...
        doc.autoTable({
          ...
          body,
          styles: { fontSize: 8, cellPadding: 1.8, lineColor: [180, 180, 180], lineWidth: 0.2, textColor: [0, 0, 0], ...(cjkFont && { font: cjkFont }) },
          headStyles: { fillColor: [225, 245, 215], textColor: [30, 30, 30], fontStyle: 'bold', halign: 'center', fontSize: 8, ...(cjkFont && { font: cjkFont }) },
          alternateRowStyles: { fillColor: [252, 252, 252] },
          columnStyles: {
      ...
      // Fallback: if no expenseItems, show debit lines with old format
      if (!hasExpenseItems) {
        const debitLines = expenseLines.filter(l => l.entryType === 'debit');
        if (debitLines.length > 0) {
          const head = [['借方明細', '科目代號', '科目名稱', '摘要', '金額']];
          const body = debitLines.map((l, idx) => [
            String(idx + 1), l.accountingCode || '', l.accountingName || '', l.summary || '', Number(l.amount).toLocaleString(),
          ]);
          const debitTotal = debitLines.reduce((s, l) => s + Number(l.amount), 0);
          body.push(['', '', '', '小計', debitTotal.toLocaleString()]);
          doc.autoTable({
            startY: y, head, body,
            styles: { fontSize: 8, cellPadding: 1.8, lineColor: [180, 180, 180], lineWidth: 0.2, textColor: [0, 0, 0], ...(cjkFont && { font: cjkFont }) },
            headStyles: { fillColor: [210, 225, 245], textColor: [30, 30, 30], fontStyle: 'bold', halign: 'center', fontSize: 8, ...(cjkFont && { font: cjkFont }) },
            alternateRowStyles: { fillColor: [252, 252, 252] },
            columnStyles: { 0: { cellWidth: 10, halign: 'center' }, 1: { cellWidth: 22 }, 2: { cellWidth: 36 }, 3: { cellWidth: 'auto' }, 4: { cellWidth: 28, halign: 'right' } },
            didParseCell: (data) => { if (data.row.index === body.length - 1) { data.cell.styles.fillColor = [235, 235, 235]; data.cell.styles.fontStyle = 'bold'; } },
            margin: { left: margin, right: margin },
          });
          y = doc.lastAutoTable.finalY + 3;
        }
  ...
  if (productMap && productMap.size > 0 && sortedDates.length > 0) {
    ...
    for (let batch = 0; batch < totalDateBatches; batch++) {
      ...
      const bodyData = [];
      products.forEach(([, entry], rowIdx) => {
        const fmtQty = v => Number.isInteger(v) ? String(v) : parseFloat(v.toFixed(1)).toString();
        const row = [
          entry.warehouse || warehouseDisplay,
          entry.name,
          entry.unitPrice.toLocaleString(),
          ...batchDates.map(d => { const q = entry.dateQty.get(d); return q ? fmtQty(q) : ''; }),
        ];
        if (isLastBatch) {
          const subtotal = Math.round(entry.totalAmount);
          row.push(fmtQty(entry.totalQty), subtotal.toLocaleString(), rowIdx === 0 ? grandTotal.toLocaleString() : '');
        }
        bodyData.push(row);
      });

      const warehouseW = 14, nameW = 34, priceW = 14, dateW = 8, qtyW = 10, subtotalW = 18, totalW = 20;
      ...
      if (isLastBatch) {
        const li = colWidths.length - 1;
        columnStyles[li]     = { cellWidth: totalW,    halign: 'right', fontStyle: 'bold', textColor: [139, 0, 0] };
        columnStyles[li - 1] = { cellWidth: subtotalW, halign: 'right' };
        columnStyles[li - 2] = { cellWidth: qtyW,      halign: 'center' };
      }
      doc.setTextColor(0, 0, 0);
      doc.autoTable({
        ...
        margin: { left: margin, right: margin },
        didParseCell: (data) => {
          if (!isLastBatch || data.section !== 'body') return;
          const li = colWidths.length - 1;
          // 總計 欄：只第一行有值，其餘行字色設白（隱形）
          if (data.column.index === li && data.row.index > 0) {
    ...
    // ===== Price comparison table (all products, 1-year window) =====
    if (priceNoteItems.length > 0) {
      const pageHeight2 = doc.internal.pageSize.getHeight();
      if (y + 50 > pageHeight2 - 15) { doc.addPage(); if (cjkFont) doc.setFont(cjkFont, 'normal'); y = 12; }
      y += 5;
      ...
      doc.text('歷史價格比較（近 1 年同廠商）', margin + 1, y);
      doc.setFontSize(7);
      doc.setFont(undefined, 'normal');
      doc.setTextColor(160, 0, 0);
      doc.text('▲ 紅字：本次單價高於歷史最低價', margin + contentWidth - 1, y, { align: 'right' });
      doc.setTextColor(0, 0, 0);
      y += 4;

      const priceHead = [['品名', '本次單價', '1年內最低價', '差異金額', '差異%', '最低價日期']];
      const priceBody = priceNoteItems.map(note => [
        note.productName,
        note.currentPrice != null ? note.currentPrice.toLocaleString() : '-',
        note.recentMin != null ? note.recentMin.toLocaleString() : '(無紀錄)',
        note.hasCheaperHistory && note.priceDiff ? note.priceDiff : '-',
        note.hasCheaperHistory && note.diffRate ? note.diffRate : '-',
        note.hasCheaperHistory && note.cheapestDate ? note.cheapestDate : '-',
      ]);
      ...
      doc.autoTable({
        ...
        head: priceHead,
        body: priceBody,
        styles: { fontSize: 7.5, cellPadding: 1.5, lineColor: [200, 200, 200], lineWidth: 0.2, textColor: [0, 0, 0], ...(cjkFont && { font: cjkFont }) },
        headStyles: { fillColor: [240, 240, 240], textColor: [60, 60, 60], fontStyle: 'bold', halign: 'center', fontSize: 7.5, ...(cjkFont && { font: cjkFont }) },
        ...
        didParseCell: (data) => {
          ...
          const note = priceNoteItems[data.row.index];
          if (!note || !note.hasCheaperHistory) {
            data.cell.styles.textColor = [255, 255, 255]; // invisible
          } else {
            data.cell.styles.textColor = data.column.index >= 1 ? [160, 0, 0] : [0, 0, 0];
