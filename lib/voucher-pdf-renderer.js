...
export function renderVoucherTablePage(doc, opts) {
  ...
  if (productMap && productMap.size > 0 && sortedDates.length > 0) {
    ...
    for (let batch = 0; batch < totalDateBatches; batch++) {
      ...
      const head = [['館別', '品名', '單價', ...dateCols, ...(isLastBatch ? ['數量', '小計'] : [])]];
      const bodyData = [];
      ...
      if (isLastBatch) bodyData.push(['', '', '', ...batchDates.map(() => ''), '', `合計 ${grandTotal.toLocaleString()}`]);

      const warehouseW = 16, nameW = 36, priceW = 16, dateW = 8, qtyW = 10, subtotalW = 22;
      const colWidths = [warehouseW, nameW, priceW, ...batchDates.map(() => dateW)];
      if (isLastBatch) colWidths.push(qtyW, subtotalW);
      const columnStyles = {};
      colWidths.forEach((w, i) => { columnStyles[i] = { cellWidth: w, halign: i >= 3 ? 'center' : (i === 2 ? 'right' : 'left') }; });
      if (isLastBatch) {
        const lastIdx = colWidths.length - 1;
        columnStyles[lastIdx] = { cellWidth: subtotalW, halign: 'right', fontStyle: 'bold' };
        columnStyles[lastIdx - 1] = { cellWidth: qtyW, halign: 'center' };
      }
      doc.setTextColor(0, 0, 0);
      doc.autoTable({
    ...
    if (priceNoteItems.length > 0) {
      ...
      const priceBody = priceNoteItems.map(note => [
        note.productName,
        note.currentPrice != null ? `$${note.currentPrice}` : '-',
        note.recentMin != null ? `$${note.recentMin}` : '(無紀錄)',
      ...
      doc.autoTable({
        ...
        headStyles: { fillColor: [245, 245, 245], textColor: [80, 80, 80], fontStyle: 'bold', halign: 'center', fontSize: 7.5, ...(cjkFont && { font: cjkFont }) },
        columnStyles: {
          0: { cellWidth: 40 },
        ...
        didParseCell: (data) => {
          if (data.section === 'body') {
            ...
            if (!note || !note.hasCheaperHistory) {
              // White text = invisible for non-problem rows
      ...
      doc.setTextColor(150, 150, 150);
      doc.text('※ 歷史價格比較（近1年同廠商紀錄），僅供參考，不影響本次進貨金額。黑色字：本次較貴；白色字：無較低歷史紀錄。', margin + 1, y);
      doc.setTextColor(0, 0, 0);
    }
  }
