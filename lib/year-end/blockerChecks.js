...
export async function checkYearEndBlockers(prisma, year, options = {}) {
  ...
  // 單次批次查詢取代 N 館別各一次 findMany
  const whNames = warehouses.map(w => w.name);
  const allWhStatuses = whNames.length > 0
    ? await prisma.monthEndStatus.findMany({
        where: { year, warehouse: { in: whNames }, status: { in: ['已結帳', '已鎖定'] } },
        select: { warehouse: true, month: true },
      })
    : [];
  const closedByWh = new Map();
  for (const row of allWhStatuses) {
    if (!closedByWh.has(row.warehouse)) closedByWh.set(row.warehouse, new Set());
    closedByWh.get(row.warehouse).add(row.month);
  }
  const whBlockers = [];
  for (const wh of warehouses) {
    const closedSet = closedByWh.get(wh.name) ?? new Set();
    if (closedSet.size < 12) {
