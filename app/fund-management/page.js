  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {/* 負餘額警示 */}
        {accounts.filter(a => Number(a.currentBalance) < 0).length > 0 && (
          <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
            ...
            <div>
              <p className="text-sm font-medium text-red-700">
                {accounts.filter(a => Number(a.currentBalance) < 0).length} 個帳戶餘額為負值
              </p>
              <p className="text-xs text-red-600 mt-0.5">
                {accounts.filter(a => Number(a.currentBalance) < 0).map(a => `${a.name} (${Number(a.currentBalance).toLocaleString()})`).join('、')}
              </p>
            </div>
          </div>
        )}
