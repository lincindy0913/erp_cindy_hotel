    ...
    <table className="w-full text-sm border-collapse">
      <thead className="sticky top-0 z-10 bg-slate-50">
        <tr className="bg-slate-50">
          {headers.map((h, i) => (
            <th key={i} className={`p-2 border border-slate-200 ${i === 0 ? 'text-left' : 'text-right'}`}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {rows.map((row, i) => (
          <tr key={i} className="hover:bg-slate-50">
            {row.map((cell, j) => (
              <td key={j} className={`p-2 border border-slate-200 ${j === 0 ? '' : 'text-right'}${j === row.length - 1 ? ' font-medium' : ''}`}>{cell}</td>
            ))}
          </tr>
        ))}
      </tbody>
      ...
      return (
        <div className="space-y-4">
          ...
          {data.bySupplier?.length > 0 && (
            <div><h4 className="text-sm font-semibold text-slate-700 mb-2">依廠商</h4>
              <SimpleTable
                headers={['廠商', '筆數', '金額', '稅額', '含稅總計']}
                rows={data.bySupplier.map(r => [r.name, r.count, `$${formatNumber(r.amount)}`, `$${formatNumber(r.tax)}`, `$${formatNumber(r.total)}`])}
              /></div>
          )}
          {data.byWarehouse?.length > 0 && (
            <div><h4 className="text-sm font-semibold text-slate-700 mb-2">依館別</h4>
              <SimpleTable
                headers={['館別', '筆數', '金額', '稅額', '含稅總計']}
                rows={data.byWarehouse.map(r => [r.name, r.count, `$${formatNumber(r.amount)}`, `$${formatNumber(r.tax)}`, `$${formatNumber(r.total)}`])}
              /></div>
          )}
        </div>
      );
      ...
      return (
        <div className="space-y-4">
          ...
          {data.byStatus?.length > 0 && (
            <div><h4 className="text-sm font-semibold text-slate-700 mb-2">依狀態</h4>
              <SimpleTable headers={['狀態', '筆數', '總計']} rows={data.byStatus.map(r => [r.name, r.count, `$${formatNumber(r.total)}`])} /></div>
          )}
          {data.byWarehouse?.length > 0 && (
            <div><h4 className="text-sm font-semibold text-slate-700 mb-2">依館別</h4>
              <SimpleTable headers={['館別', '筆數', '總計']} rows={data.byWarehouse.map(r => [r.name, r.count, `$${formatNumber(r.total)}`])} /></div>
          )}
        </div>
      );
      ...
      return (
        <div className="space-y-4">
          ...
          {data.byCategory?.length > 0 && (
            <div><h4 className="text-sm font-semibold text-slate-700 mb-2">依類別</h4>
              <SimpleTable headers={['類別', '筆數', '總計']} rows={data.byCategory.map(r => [r.name, r.count, `$${formatNumber(r.total)}`])} /></div>
          )}
          {data.byWarehouse?.length > 0 && (
            <div><h4 className="text-sm font-semibold text-slate-700 mb-2">依館別</h4>
              <SimpleTable headers={['館別', '筆數', '總計']} rows={data.byWarehouse.map(r => [r.name, r.count, `$${formatNumber(r.total)}`])} /></div>
      ...
      return (
        <div className="space-y-4">
          ...
          {data.byAccountType?.length > 0 && (
            <div><h4 className="text-sm font-semibold text-slate-700 mb-2">依帳戶類型</h4>
              <table className="w-full text-sm border-collapse">
                <thead className="sticky top-0 z-10 bg-slate-50">
                  <tr className="bg-slate-50">
                    {['帳戶類型', '收入', '支出', '移轉', '淨額'].map(h => (
                      <th key={h} className={`p-2 border border-slate-200 ${h === '帳戶類型' ? 'text-left' : 'text-right'}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
            ...
            <details className="mt-2">
              ...
              <table className="w-full text-xs border-collapse mt-2">
                <thead className="sticky top-0 z-10 bg-slate-50">
                  <tr className="bg-slate-50">
                    {['層級', '科目群', '收入', '支出'].map(h => (
                      <th key={h} className={`p-1.5 border border-slate-200 ${h === '層級' || h === '科目群' ? 'text-left' : 'text-right'}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
